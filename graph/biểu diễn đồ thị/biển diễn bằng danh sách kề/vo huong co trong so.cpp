...
signed main() {
    ...
    cin >> n >> m;
    v(value) a; /// danh sách canh đồ thị vô hướng có trọng số
    forr(i, 1, m, 1) { /// m cạnh liên thông
        ll u, v, value; cin >> u >> v >> value; /// cạnh liên thông u - v, c(u, v) = value
        a.pb({u, v, value}); /// danh sách cạnh a, từ đỉnh u đến v có c = value
        a.pb({v, u, value}); /// danh sách cạnh a, từ v đến u có c = value
    }
    for (auto e : a) {
        cout << e.start << ' ' << e.end << ' ' << e.w << '\n';
    }
    return 0;
}
