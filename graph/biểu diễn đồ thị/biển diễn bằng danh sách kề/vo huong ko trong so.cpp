...
ll n, m;
vll a[1000007]; /// danh sách canh đồ thị vô hướng ko trọng số
...
signed main() {
    ...
    forr(i, 1, m, 1) { /// m cạnh liên thông
        ll u, v; cin >> u >> v; /// cạnh liên thông u - v
        a[u].pb(v); /// danh sách cạnh a, từ đỉnh u - v
        a[v].pb(u); /// danh sách cạnh a, từ đỉnh v - u
    }
    forr(i, 1, n, 1) {
        cout << i << ": ";
        for (ll v : a[i]) {
