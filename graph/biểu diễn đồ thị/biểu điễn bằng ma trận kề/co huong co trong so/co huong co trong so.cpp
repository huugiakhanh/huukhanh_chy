...
/// đồ thị có hướng có trọng soo n đỉnh
ll n, m;
...
signed main() {
    ...
    cin >> n >> m;
    vvll(dothi, n + 2, n + 2, 0); /// ma trận n đỉnh kích thước n x n
    forr(i, 1, m, 1) { /// m cạnh liên thông
        ll u, v, value; cin >> u >> v >> value; /// đỉnh u liên thông đến đỉnh v hướng u đến vê trọng số c(u, v) = value
        dothi[u][v] = value; /// từ tỉnh u đến đc đỉnh v hướng u, v trọng số c(u, v) = vaule
        /// dothi[i][j] == value tức là có cạnh liên thông hướng i đến j hướng i, j trọng số c(u, v) = value
        /// dothi[i][j] == 0 tức là ko cạnh liên thông
    }
    forr(i, 1, n, 1) {
        forr(j, 1, n, 1) {
            cout << dothi[i][j] << " ";
