...
void input() noexcept(true) {
    ss.pub("1. Gia đình khỏe mạnh, hạnh phúc");
    ss.pub("2. Ôn đội tuyển để có thể tham gia các kì thi tháng 4-5, 6-8");
    ss.pub("3. Nếu thành công có thể tham gia đội tuyển Học sinh giỏi quốc gia tháng 9/2026");
    ss.pub("4. Chỉ cần có giải ba - khuyến khích HSGQG tin học 2026");
    ss.pub("5. Cùng Nguyệt đi hết năm 2026 thật trọn vẹn, ít xích mích hơn");
    ss.pub("6. Mong code ít bug hơn, 1 đấm AC");
    TIMEi;
}
void output() noexcept(true) {
    forr(i, 1, 10, 1) cout << "-"; cout << " khai code đầu năm 2026 "; forr(i, 1, 10, 1) cout << "-";
    cout << '\n'; forr(i, 1, 44, 1) cout << '-'; cout << '\n' << '\n';
    cout << "Con tên là: Nguyễn Hữu Gia Khánh \n" << "Sinh ngày 02 thánh 02 năm 2010 \n";
    cout << '\n' << "Các ước dự định của con năm nay: " << '\n' << '\n';
    for_Cout(ss, '\n');
    cout << '\n' << "---Cầu mong năm nay là 1 năm học thành công---" << '\n';
    TIMEo;
}
int main() {
    fastIO();
