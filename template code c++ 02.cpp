...
template<typename value> void read(value &x) { x = 0; int sign = 1, c = getchar(); while (c != '-' && (c < '0' || c > '9')) { c = getchar(); } if (c == '-') { sign = -1, c = getchar(); } while (c >= '0' && c <= '9') { x = x * 10 + (c - '0'); c = getchar(); } x *= sign; }
template<typename value> void write(value x) {if (x < 0) { putchar('-'); x = -x; } if (x > 9) { write(x / 10); } putchar(char('0' + x % 10)); }
...
template<typename... value> void inallf(value&... valueofvalue) { ((read(valueofvalue)), ...);}
template<typename... value> void outallf(char valueofchar, const value&... valueofvalue) { ((write(valueofvalue), putchar(valueofchar)), ...); }
